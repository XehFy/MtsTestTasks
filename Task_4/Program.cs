 ...
 * 
 * Для хранения счетчика количества вхождений элементов в список использую словарь по причине описаной ниже
 * 
 * 
 * 
 * Для обязательного по сигнатуре параметра "sortFactor" я не смог придумать лучшего применения, чем создание с его помощью словаря, 
 ...
 * сведется к тому, что sortFactor - это разность между максимальным и минимальным элементом, которые могут быть в потоке.
 * Исходя из этого можно сразу узнать диапазон для ключей словоря, что позволяет создать словарь минимального размера.
 * 
 * Сложность алгоритма по времени получается линейная O(n + 2k), где n - количество эл-ов входного потока, k - количество возможных значений в потоке
